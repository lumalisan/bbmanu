...
// Si es bloqueante, la condición del do/while tiene que ser bloqueante = 1
// Si no, devuelve TECLA_NO_PULSADA (255, unsigned char)
unsigned char getKey(unsigned char block){
		...
		if(ROW2 == 0) return  8;
		if(ROW3 == 0) return 11;
		} while (block == NO_BLOQUEANTE);

		return TECLA_NO_PULSADA;
