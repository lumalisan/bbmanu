...
//	C1RX1CONbits.RXFUL = 0;
	...
	// Configure acceptance masks of buffers 0 and 1 (we set the same mask for both buffers)
	C1RXM0SIDbits.SID	 = maskSID;
	C1RXM1SIDbits.SID	 = maskSID;
					   //= 0b11111111111;	

	C1RXM0SIDbits.MIDE   = 1; // Match only SID or EXSIDE as configured by EXIDE in filters of rxbuff0
	C1RXM1SIDbits.MIDE   = 1; // Match only SID or EXSIDE as configured by EXIDE in filters of rxbuff1
	

	// Configure acceptance filters for rxbuffers 0 and 1

		// First enable all filters for standard indentifier

	C1RXF0SIDbits.EXIDE	= 0;	// Filters 0 and 1 apply to rxbuff0
	C1RXF1SIDbits.EXIDE	= 0;

	C1RXF2SIDbits.EXIDE	= 0;	// Filters 2 to 5 apply to rxbuff0
	C1RXF3SIDbits.EXIDE	= 0;
	C1RXF4SIDbits.EXIDE	= 0;
	C1RXF5SIDbits.EXIDE	= 0;


		// Second set filter values (max SID)

	C1RXF0SIDbits.SID	= filterSID;	// Filters 0 and 1 apply to rxbuff0
	C1RXF1SIDbits.SID	= filterSID;

	C1RXF2SIDbits.SID	= 0b1111111111;	// Filters 2 to 5 apply to rxbuff0
	C1RXF3SIDbits.SID	= 0b1111111111;
	C1RXF4SIDbits.SID	= 0b1111111111;
	C1RXF5SIDbits.SID	= 0b1111111111;
